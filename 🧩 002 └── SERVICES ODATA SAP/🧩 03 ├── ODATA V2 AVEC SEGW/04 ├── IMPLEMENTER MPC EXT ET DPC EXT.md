# 4. IMPLÉMENTER MPC_EXT ET DPC_EXT

## 4.A RÉSULTAT ATTENDU

Placer les extensions de modèle dans `MPC_EXT`[^terme-mpc] et les opérations de données dans `DPC_EXT`[^terme-dpc].
...
### 4.E.4 ÉTAPE 4 — RETOURNER OU LEVER UNE ERREUR
...
## 4.F SQUELETTE DE LECTURE PAR CLÉ
...
[^terme-mpc]: **MPC.** Model Provider Class générant le modèle et les métadonnées. Voir [le lexique](<../🧩 00 ├── LEXIQUE ODATA ET SAP GATEWAY/04 ├── SEGW ET RUNTIME V2.md#mpc>).
[^terme-dpc]: **DPC.** Data Provider Class fournissant les méthodes d’accès et de modification des données. Voir [le lexique](<../🧩 00 ├── LEXIQUE ODATA ET SAP GATEWAY/04 ├── SEGW ET RUNTIME V2.md#dpc>).
