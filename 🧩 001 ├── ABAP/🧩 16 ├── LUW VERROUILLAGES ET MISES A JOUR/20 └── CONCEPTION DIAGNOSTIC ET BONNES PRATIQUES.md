...
- exécuter un commit dans une méthode[^terme-methode] profonde ou un exit ;
...
- appeler un module en update task[^terme-update-task] sans garantir le commit ;
- relancer une erreur `SM13`[^outil-sm13] sans contrôler l’idempotence ;
- supprimer un verrou `SM12`[^outil-sm12] sans identifier le propriétaire ;
- mélanger effets externes et transaction locale sans stratégie de compensation.

## 20.D CHECKLIST

- [ ] Unité métier et frontière de SAP LUW[^terme-sap-luw] définies
...
- [ ] Diagnostic `SM12`, `SM13`, `ST22`[^outil-st22] et logs testé
...
Identifier l’entrée utilisateur ou job[^terme-job], les clés métier, les lectures, les verrous, les écritures directes, les modules de mise à jour et la borne finale. Dessiner l’ordre réel des opérations. Le diagnostic doit porter sur l’unité complète, pas uniquement sur l’instruction qui a signalé l’erreur.

### 20.E.2 ÉTAPE 2 — CONTRÔLER LES BORNES ET LE PROPRIÉTAIRE DU COMMIT

Rechercher tous les commits et rollbacks du code Z traversé, puis examiner le contrat des API[^terme-api] standard appelées. Vérifier qu’un seul orchestrateur décide de la fin de l’unité métier. Identifier tout effet externe qui ne peut pas être annulé par la transaction ABAP[^terme-abap].
...
Corriger la cause prouvée, puis reprendre avec les mêmes paramètres et une clé de test contrôlée. Vérifier la cohérence finale, l’absence de doublons, la libération des verrous et l’absence d’update résiduelle en erreur. Conserver les identifiants de trace[^terme-trace] et les états avant/après.
