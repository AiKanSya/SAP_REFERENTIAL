...
Dans un contexte où un programme de contrôle manipule des identifiants, montants, dates, statuts et structures dont le typage doit rester explicite, le besoin consiste à **déclarer et utiliser variables avec `data` avec un typage explicite dans un programme ABAP**. Cette notion est pertinente lorsque le lecteur doit pouvoir relier la syntaxe ou l’outil à une situation professionnelle concrète.
...
- À l’issue du chapitre, le lecteur sait **déclarer et utiliser variables avec `data` avec un typage explicite dans un programme ABAP**.
...
➡️ [Chapitre suivant — CONSTANTES ET LITTÉRAUX](<./05 - 🍧 CONSTANTES ET LITTERAUX.md>)
