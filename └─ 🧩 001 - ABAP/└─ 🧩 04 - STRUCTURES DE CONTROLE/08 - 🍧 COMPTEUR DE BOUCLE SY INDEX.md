...
Dans un contexte où un traitement doit appliquer plusieurs règles métier et arrêter ou poursuivre le flux selon les données rencontrées, le besoin consiste à **piloter le flux d’exécution avec compteur de boucle sy-index tout en conservant un code lisible et borné**. Cette notion est pertinente lorsque le lecteur doit pouvoir relier la syntaxe ou l’outil à une situation professionnelle concrète.
...
- À l’issue du chapitre, le lecteur sait **piloter le flux d’exécution avec compteur de boucle sy-index tout en conservant un code lisible et borné**.
...
➡️ [Chapitre suivant — FILTRER UNE ITÉRATION AVEC CHECK ET CONTINUE](<./09 - 🍧 FILTRER UNE ITERATION AVEC CHECK ET CONTINUE.md>)
